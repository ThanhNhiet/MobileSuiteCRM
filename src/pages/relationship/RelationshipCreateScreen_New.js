...
export default function RelationshipCreateScreen_New() {
    ...
    if (!relaFor) {
        throw new Error('relaFor is required for RelationshipCreateScreen_New');
    }
    ...
    const {
        ...
        toggleBoolField,
        recordForRela
    } = useRelationshipCreate(moduleName, relaFor);
    ...
    const renderFormFields = () => {
        return createFields.map((field) => {
            ...
            if (isRelateField(field.key)) {
                ...
                //console.log("Related module for field", field.key, "is", relatedModuleName);
                //console.log("Related module data:", relateModuleData[field.key]);
                if (recordForRela.moduleName === relatedModuleName) {
                return (
                    <View key={field.key} style={styles.row}>
                        {renderFieldLabel(field.key)}
                        <View style={styles.relateValueBox}>
                            <Text style={[
                                styles.value,
                                !recordForRela.name && styles.placeholderText,
                                styles.relateValueText
                            ]}>
                                {recordForRela.name}
                            </Text>
                        </View>
